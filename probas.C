...
//                 return;
//		 cout<<4<<endl;
		  cout << "*** probas.C: use KDE" <<endl;

		TString hkde = hnameS + TString("_smoothed_hist_from_KDE");
		  sigF = (TH1*)instDir->Get( hkde );

		 if (sigF) {
                     bkgF = (TH1*)instDir->Get( hkde.ReplaceAll("_tr_S","_tr_B") );
                  }

		 if ((sigF == NULL || bkgF == NULL) &&!hname.Contains("hist") ) {
                  cout << "*** probas.C: big troubles - did not found histogram " << hkde.Data() << " "
                       << sigF << " " << bkgF << endl;
			return;
	}
...
//		cout<<5<<endl;
                ...
                if (drawData)     sig->Draw("samee");
	if (drawData)                 bgd->Draw("samee");
                ...
                if (drawData)     legend->AddEntry(sig,"Signal data","P");
                     legend->AddEntry(sigF,"Signal PDF","L");
                 if (drawData)    legend->AddEntry(bgd,"Background data","P");
