...
void variables( TString fin = "TMVA.root", TString dirName = "InputVariables_Id", TString title = "TMVA Input Variables", TString outputdir="plots",
...
TMVA::gConfig().GetVariablePlotting().fNbins1D = 25.0;
...
//	if (sig) sig->Smooth(50);
//	if (bgd) bgd->Smooth(250);
      ...
      // save canvas to file
      if (countPad%noPadPerCanv==0) {
         TString fname = Form( "%s/%s_c%i",outputdir.Data(), outfname.Data(), countCanvas );
         TMVAGlob::plot_logo();
