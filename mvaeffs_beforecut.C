...
////END
      ...
      sigE->SetTitle( Form(" \'Before-Cut\' efficiencies for %s classifier", methodTitle.Data()) );
...
void StatDialogMVAEffs::UpdateCanvases(TString outputdir) 
{
   ...
   while ( (info = (MethodInfo*)next()) ) {
      info->canvas->Update();
      if (fFormula=="B/S") info->rightAxis->SetWmax(6);
...
///Calculate 'before-cut' efficiencies
         ...
         Float_t B = (1e0- info->origBgdE->GetBinContent( i )) * fNBackground;
         info->purS->SetBinContent( i, (S+B==0)?0:S/(S+B) );
         info->purB->SetBinContent( i, (S+B==0)?0:B/(S+B) );

//TFormula f("sigf",GetFormula());
//	cout<<"f.Eval(S,B) = "<<f.Eval(S,B)<<endl;
         if (f.Eval(S,B)>6) info->sSig->SetBinContent( i, 6 );
         else info->sSig->SetBinContent( i, f.Eval(S,B) );
         info->effpurS->SetBinContent( i, eS*info->purS->GetBinContent( i ) );
         info->effpurB->SetBinContent( i, (1e0- info->origBgdE->GetBinContent( i ))*info->purB->GetBinContent( i ) );
///END
      }
...
void StatDialogMVAEffs::DrawHistograms(TString outputdir) 
{
   ...
   while ( (info = (MethodInfo*)next()) ) {
      ...
      info->effpurS->SetTitle("\'Before - Cut\' efficiencies and optimal cut value");
	...
	info->effpurS->GetXaxis()->SetTitle( info->methodTitle + " output" );
      ...
      info->purB->Draw("samehistl");      
      info->effpurB->Draw("samehistl");      
      ...
      legend2->AddEntry(info->sSig,Form("%s",fFormula.Data()),"L");
      legend2->Draw("same");
...
Float_t eS;
 ...
 Float_t B;
	...
	if (purB_<0 ) {
	maxbin= info->sSig->GetMaximumBin();
	maxSignif=info->maxSignificance;

      eS = 1e0 - info->origSigE->GetBinContent( maxbin );
          S = eS * fNSignal;
          B = (1e0- info->origBgdE->GetBinContent( maxbin )) * fNBackground;


	}
else {
///Find the bin for bkgPurtity

  TH1 * purB = info->purB;
        for (Int_t i=1; i<purB->GetNbinsX();i++) {
                double content = purB->GetBinContent(i);
                if (content<purB_ && i!=1) { 
			maxbin=i-1; 
				break; 
		}
...
TFormula f("sigf",GetFormula());
     maxSignif=f.Eval(S,B);
//      cout<<"maxSignif = "<<f.Eval(S,B)<<endl;

}

      info->line1 = tl.DrawLatex( 0.15, 0.23, Form("For %1.0f signal and %1.0f background", fNSignal, fNBackground));
      tl.DrawLatex( 0.15, 0.19, Form("events the  %s is",GetLatexFormula().Data()));
	...
	tl.DrawLatex(0.13, 0.74, "different signal efficiency. Shown is the purity for each cut selection.");
      ...
      if (Save_Images) {
         TMVAGlob::imgconv( c, Form("%s/mvaeffs_%s", outputdir.Data() , info->methodTitle.Data()) ); 
      }
      countCanvas++;
   }
}

void StatDialogMVAEffs::PrintResults( const MethodInfo* info, double bkgPurity)
{
   Int_t maxbin ;
   double  maxSignif;	
 Float_t eS;
...
//	maxSignif=info->sSig->GetBinContent(maxbin);
//	cout<<"maxSignif = "<<maxSignif<<endl;
...
//	cout<<"maxSignif = "<<maxSignif<<endl;
   ...
   cout << "--- " 
        << Form( opt.Data(),
                 ...
                 S,   
                 B,
                 1e0-info->origSigE->GetBinContent( maxbin ),
                 1e0-info->origBgdE->GetBinContent( maxbin ) )
        << endl;
}

void mvaeffs_beforecut( TString fin = "TMVA.root", 
	TString formula="S/sqrt(S+B)",TString outputdir="plots", Bool_t raiseNum=kTRUE,
         TString purB="-1",
