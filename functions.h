...
///Read file containig processed by TSelector 
void Readinput_v2(const char *nameFile, std::vector<std::string> & listFiles, std::vector<std::string> & process,std::vector<std::string> & type, std::vector<double> & xsects, std::vector<double> & effs ,std::vector<double> & wgts ,std::vector<int> & mergeList,int &Number) {
ifstream  infile;
infile.open(nameFile,std::ios::in);
if (!infile) {std::cout << "Couldn't read the file"; return;}
Number=0;
