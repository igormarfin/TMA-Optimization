...
#include "HowManyFieldsInFile.C"

void TMVAClassificationCreator( TString list_of_files="list_of_files",TString myMethodList = "", TString _treeName="", TString _exclusion="", TString _options_training="", TString _train_test_options = "", TString fileout="" ) ///usage:
{
...
TString _nameMVA="";
   ...
   TString outfileName;
   if (fileout.Length()>0) outfileName=fileout;
   else outfileName=TString("TMVA.root");
...
{
        cout<<"1 file: "<<((TFile*) gROOT->GetListOfFiles()->At(j))->GetName()<<endl;

         ((TFile*) gROOT->GetListOfFiles()->At(j))->cd();
	if ( (!TString(gFile->GetName()).Contains("TMVA")) && (!TString(gFile->GetName()).Contains("mhmax_7")) ) break;
}
...
///Let's fill variables_to_use
	for (Int_t jjj=0;jjj<tree->GetListOfBranches()->GetSize();jjj++)
	{
		...
		std::string str(obj->GetName());
  	       if (str!="mva") variables_to_use[str]=1;
}
	}
...
for (Int_t j=0;j<gROOT->GetListOfFiles()->GetEntries();j++)
{
	 ((TFile*) gROOT->GetListOfFiles()->At(j))->cd();
         ...
         cout<<"file: "<<((TFile*) gROOT->GetListOfFiles()->At(j))->GetName()<<endl;


	 if (TString(gFile->GetName()).Contains("TMVA")) continue;
	if (TString(gFile->GetName()).Contains("out.mhmax_7")) continue;
...
cout<<"_train_test_options= "<<_train_test_options<<endl;
...
else test_options =  "nTrain_Signal=0:nTrain_Background=0:SplitMode=Random:SplitSeed=100:NormMode=NumEvents:!V";
...
/*
 TMVA::IMethod *  method=factory->GetMethod(_nameMVA);
 if (method)
{
	TMVA::MethodBDT * method2 = dynamic_cast<TMVA::MethodBDT*>(method);
//	if (method2) method2->OptimizeTuningParameters("ROCIntegral","FitGA");
} 
*/
