...
void mvas( TString fin = "TMVA.root", TString outputdir="plots",  HistType htype = MVAType, Bool_t useTMVAStyle = kTRUE )
{
...
//   cout<<file->GetListOfKeys()->Print()<<endl;

   TIter next(file->GetListOfKeys());
   TKey *key(0);   
   while ((key = (TKey*)next())) {

//		cout<<titkey->GetName()<<endl;
//        cout<<gROOT->GetClass(titkey->GetClassName())<<endl;
	...
	cout<<sig->Integral("width")<<endl;
	cout<<bgd->Integral("width")<<endl;

	 cout<<sig->GetNbinsX()<<endl;
	 cout<<bgd->GetNbinsX()<<endl;
...
//       bgd->Smooth(8050);
       ...
       bgdOv->Rebin(2);
         ...
         if (Save_Images) {
            if      (htype == MVAType)     TMVAGlob::imgconv( c, Form("%s/mva_%s", outputdir.Data() ,    methodTitle.Data()) );
            else if (htype == ProbaType)   TMVAGlob::imgconv( c, Form("%s/proba_%s", outputdir.Data() ,    methodTitle.Data()) ); 
            else if (htype == CompareType) TMVAGlob::imgconv( c, Form("%s/overtrain_%s", outputdir.Data() ,   methodTitle.Data()) ); 
            else                           TMVAGlob::imgconv( c, Form("%s/rarity_%s",  outputdir.Data() ,  methodTitle.Data()) ); 
         }
         countCanvas++;
